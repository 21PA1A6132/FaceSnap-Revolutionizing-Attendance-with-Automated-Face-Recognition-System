...
recognizer = pickle.loads(open('recognizer5.pickle', "rb").read())
le = pickle.loads(open('le5.pickle', "rb").read())

print("Starting webcam")
vs = cv2.VideoCapture('test\\test_1.mp4')  # Use 0 for the default camera
time.sleep(1)
...
while True:
    ...
    for i in range(0, face_detections.shape[2]):
        ...
        if confidence >= 0.98:
            ...
            if newpreds[j] > 0.99 and not(is_exp(newpreds[j])):
