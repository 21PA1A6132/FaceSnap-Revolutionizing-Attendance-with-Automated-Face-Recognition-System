...
recognizer = pickle.loads(open('recognizer_norm 3.pickle', "rb").read())
le = pickle.loads(open('le_norm 3.pickle', "rb").read())

print("Starting webcam")
vs = cv2.VideoCapture('test\\test2.mp4')  # Use 0 for the default camera
time.sleep(1)
...
while True:
    ...
    for i in range(0, face_detections.shape[2]):
        ...
        if confidence >= 0.90:
            ...
            if newpreds[j] > 0.97 and not(is_exp(newpreds[j])):
